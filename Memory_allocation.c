...
// Simulation loop
double simulationLoop(Block memory[], int memorySize, double averageRequestSize, double stddevRequestSize, int (*allocationStrategy)(Block[], int, int)) {
    int holesExamined = 0;
    int requestCount = 0;
    int lastAllocated = 0;
    ...
    while (1) {
        ...
        int allocatedBlock = allocationStrategy(memory, MEMORY_SIZE, requestSize);
        if (allocatedBlock != -1) {
